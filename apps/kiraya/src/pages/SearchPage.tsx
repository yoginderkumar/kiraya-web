import { useSearchProducts } from '@kiraya/data-store/products';
import {
  Box,
  DataLoadingFallback,
  Heading,
...
import { Categories, getCategoryDetails } from '../Products/data';
...
export default function SearchPage() {
  return (
    <ErrorBoundary>
      <SuspenseWithPerf
        fallback={<DataLoadingFallback label="fetching products..." />}
        traceId="loading_book_details"
      >
        <SearchDashboard key={'test'} />
      </SuspenseWithPerf>
    </ErrorBoundary>
  );
}
...
function SearchDashboard() {
  ...
  const { products, setParamValue } = useSearchProducts();
  const searchForText = useMemo(() => {
    let renderText = 'nothing';
    const category = searchParams.get('category');
    if (category?.length) {
      renderText = getCategoryDetails(category as Categories)?.label || '';
    }
  ...
  useEffect(() => {
    if (searchParams.get('query')?.length) {
      setParamValue('query', searchParams.get('query'));
    }
  }, [searchParams, setParamValue]);

  useEffect(() => {
    if (searchParams.get('category')?.length) {
      setParamValue('category', searchParams.get('category'));
    }
  }, [searchParams, setParamValue]);
